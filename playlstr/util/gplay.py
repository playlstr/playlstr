...
def gplay_search(query: str, search_type="track") -> list:
    ...
    if search_type == "track":
        ...
        result = requests.get(
            ...
            if button[0].attrib["href"][0] == "h"
            else "http://play.google.com{}".format(button[0].attrib["href"])
        )
...
def gplay_parse_track_search(html: lxml.html) -> list:
    tracks = []
    for div in html.xpath(
        "//div/c-wiz/c-wiz/c-wiz/div/div/div/c-wiz/div/div/div/div/div/div/div/div"
    ):
        if not div[0].xpath("./a"):
            continue
        ids = div[0].xpath("./a")[0].attrib["href"].split("/")[3]
        matches = re.search(
            r"^album\?id=(.*)&tid=song-(.*)$", ids
        )  # Album and song ids
        tracks.append(
            {
                "title": div[0].attrib["title"],
                "artist": div[1][0][0].text,
...
def gplay_parse_album_search(html: lxml.html) -> list:
    albums = []
    for div in html.xpath(
        "//c-wiz/c-wiz/div/div/div/c-wiz/div/div/div/div/div/div/div/div"
    ):
        if not div[0].xpath("./a"):
            continue
...
def gplay_parse_playlist_track(html: lxml.html, fetch_album_name: bool = True) -> Track:
    ...
    track = Track.objects.create(
        title=title,
        artist=artist,
        duration=duration_str_to_sec(duration),
        gplay_album_id=album_id,
        gplay_id=song_id,
    )
    if fetch_album_name:
        album_response = requests.get(
            "http://play.google.com/store/music/album?id={}".format(album_id)
        )
